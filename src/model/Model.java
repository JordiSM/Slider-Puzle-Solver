...
public class Model {
        ...
        for (int i = 0; i < this.tamañoTablero; i++) {
            for (int j = 0; j < this.tamañoTablero; j++) {
                if(this.puzzle[i][j] != (j + i * tamañoTablero)){
                    if(i != tamañoTablero -1 && j != tamañoTablero -1) return false;
                }
            }
        }
        return true;
    ...
    private Point findBlank() throws Exception{
        for (int i = 0; i < this.tamañoTablero; i++) {
            for (int j = 0; j < this.tamañoTablero; j++) {
                if(this.puzzle[i][j] == -1) return new Point(i,j);
            }
        }
        //ERROR
    ...
    public void move(Movement move) {
        ...
        switch(move) {
            ...
            default -> throw new RuntimeException("Error: Movement '" + move.name() + "' not found");
        }    
        //SWAP
